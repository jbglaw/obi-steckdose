...
const int pin_extra4 = 16;	/* Seems NC.  */
...
extra_pins_handle (void)
{
	if (last_test_millis + 1500 < millis ()) {
		...
		digitalWrite (pin_extra1, !! (test_counter & 0x01));
		digitalWrite (pin_extra2, !! (test_counter & 0x02));
		digitalWrite (pin_extra3, !! (test_counter & 0x04));
		digitalWrite (pin_extra4, !! (test_counter & 0x08));
	}
